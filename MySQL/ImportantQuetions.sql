...
-- Q6>List the hotel name and city of the hotel with the highest room rate

-- SELECT HOTELNAME, CITY FROM HOTEL WHERE HOTELNO=(SELECT HOTELNO FROM ROOM WHERE RATE=(SELECT MAX(RATE) FROM ROOM));
-- Q7>List all guests with single member who have booked "The Park" Hotel for more than 2 days
SELECT E1.GUESTNAME FROM GUEST E1 INNER JOIN BOOKING E2 ON E1.GUESTNO=E2.GUESTNO
INNER JOIN HOTEL E3 ON E3.HOTELNO=E2.HOTELNO WHERE NOMEMBER=1 AND HOTELNAME='THE PARK' AND DATEDIFF(DATETO,DATEFROM)>2;
-- Q8>List the details of all rooms at the "Hyatt" Hotel, including the name of the guest staying in the room with number of members, if the room is occupied 
-- SELECT ROOM.ROOMNO, GUEST.GUESTNAME, GUEST.NOMEMBER FROM BOOKING INNER JOIN ROOM
-- ON BOOKING.ROOMNO = ROOM.ROOMNO INNER JOIN HOTEL ON BOOKING.HOTELNO = HOTEL.HOTELNO
-- INNER JOIN GUEST ON BOOKING.GUESTNO = GUEST.GUESTNO
-- WHERE HOTELNAME = 'HYATT';
-- Q9>What is the total incom from bookings for the "The Taj Bengal" Hotel in the month of January and February

-- Q10>List the rooms that are unoccupied at the "Orchid" Hotel in the month of "March"
